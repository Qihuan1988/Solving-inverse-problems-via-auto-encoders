...
for i in range(0,6):
    output1 = np.loadtxt('./Reconstructed Images/4NNs-PGD/%s/imageoutput%s.txt' % (i,image_number))
    origin = np.loadtxt('./Reconstructed Images/Original_Images_data/imageorigin%s.txt' % image_number)
    output2 = np.loadtxt('./Reconstructed Images/Unet-refine/%s/imageoutput%s.txt' % (i,image_number))
