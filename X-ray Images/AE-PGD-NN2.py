...
for cc in range(start_num,len(m)): #for different m values
    ...
    for step, (x,b_label) in enumerate(test_loader):
        ...
        '''''''''
        if Noise_exist == True:
            np.savetxt('./Reconstructed Images/NN2-PGD-10dB/%s/imageoutput%s.txt' % (cc, step), output.detach().numpy())
        else:
            np.savetxt('./Reconstructed Images/NN2-PGD/%s/imageoutput%s.txt'%(cc,step),output.detach().numpy())
